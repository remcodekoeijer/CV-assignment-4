...
int main()
{
	...
	*/
	...
	Mat img = cv::imread("data/pug_166.jpg", IMREAD_GRAYSCALE);
	namedWindow("OpenCV");
	imshow("OpenCV", img);
	...
	hog.compute(img, descriptors, hogWinStride, hogPadding, locations); 
	...
	imshow("gradient", get_hogdescriptor_visu(img.clone(), descriptors, Size(128, 128)));
